...
#	check_group(group)
	...
	var_meta <- sort_by_terms(var_meta, "soil", group)
	...
	if (!is.null(var_meta)) {
		soilf <- file.path(path, "data", "clean", group, paste0(cleanuri, "_meta_variable.csv"))
		data.table::fwrite(var_meta, soilf, row.names=FALSE)
	}
